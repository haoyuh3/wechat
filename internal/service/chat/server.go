		...
		case data := <-s.Transmit:
			{
				...
				if chatMessageReq.Type == message_type_enum.Text {
					...
					s.handleAudioOrVideoMessage(chatMessageReq)
				}
...
func (s *Server) sendTextMessageToUser(message model.Message, chatMessageReq request.ChatMessageRequest) {
	...
	// 因为send_id肯定在线，所以这里在后端进行在线回显message
...
func (s *Server) sendTextMessageToGroup(message model.Message, chatMessageReq request.ChatMessageRequest) {
...
func (s *Server) handleTextMessage(chatMessageReq request.ChatMessageRequest) {
	...
	if message.ReceiveId[0] == 'U' { // 发送给User
		s.sendTextMessageToUser(message, chatMessageReq)

	} else if message.ReceiveId[0] == 'G' { // 发送给Group
		s.sendTextMessageToGroup(message, chatMessageReq)
	}
}

func (s *Server) sendFileMessageToUser(message model.Message, chatMessageReq request.ChatMessageRequest) {
	messageRsp := respond.GetMessageListRespond{
		SendId:     message.SendId,
		SendName:   message.SendName,
		SendAvatar: chatMessageReq.SendAvatar,
...
func (s *Server) sendFileMessageToGroup(message model.Message, chatMessageReq request.ChatMessageRequest) {
	messageRsp := respond.GetGroupMessageListRespond{
		SendId:     message.SendId,
		SendName:   message.SendName,
		SendAvatar: chatMessageReq.SendAvatar,
...
func (s *Server) handleFileMessage(chatMessageReq request.ChatMessageRequest) {
	// Store message
	message := model.Message{
		Uuid:       fmt.Sprintf("M%s", random.GetNowAndLenRandomString(11)),
		SessionId:  chatMessageReq.SessionId,
	...
	if message.ReceiveId[0] == 'U' {
		s.sendFileMessageToUser(message, chatMessageReq)
	} else if message.ReceiveId[0] == 'G' {
		s.sendFileMessageToGroup(message, chatMessageReq)
	}
}

func (s *Server) handleAudioOrVideoMessage(chatMessageReq request.ChatMessageRequest) {
